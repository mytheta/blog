...
今回は，gkeのクラスタ上に立ててみたいと思います．

### helmでインストール
helmのstable chartにconourseがあるのでhelmをつかってインストールしてみたいと思います．  
...
自分の好みの設定をvalue.yamlに記述してみてください，
```
...
もしterraformを使用している場合は以下のように書いてもらえればいいかと思います，  
...
resource "google_compute_managed_ssl_certificate" "((concourse))" {
  ...
  managed {
    domains = ["concourse.${google_dns_managed_zone.xhpa-zone.dns_name}"]
  }
}
```
...
### ciの流れ
pipelineでは，以下のような流れを一つのjobとして定義していきたいと思います．

```
- githubにpushしたらjob起動
- githubのstatusをpending
- go build & go lint & go test
- 成功/失敗をslackに通知 & githubのstatusをsuccess/failure

```

### resourseの定義
Jobで使用するresourse(gitやdockerなど)を定義していきたいと思います．  
今回使用するresourseは，goのソースコード(go-concourse-src)，commitのステータス(go-concourse-status)，slack(notify)ですので，  
...
resources:
  ...
  - name: notify
    ...
    source:
      url: ((slack-url))
```
また，これらのresourseはconcourseがメンテナンスしているものではないので，resourse typeを定義しなくてはいけません．  
resourse typeを以下のように定義していきます．  
```
resource_types:
...
```
resourseに関しては，[こちら](https://github.com/concourse/concourse/wiki/Resource-Types)に，色々書かれていますので参照してみてください．  
...
getでresourceの取得，putでresourceを更新します．  
```
jobs:
  - name: go-test
    plan:
...
```
      - get: go-concourse-src
        trigger: true
```
で，go=concourseをトリガーとしてresourseを取得しています．
```
      - put: go-concourse-status
        params: { state: pending, commit: go-concourse-src }
```
こちらは，pushしたcommmitのstatusをpendingにしています．
```
      - task: go-build
        file: go-concourse-src/concourse/tasks/build.yaml
```
go-buildというtaskを実行します．  
実行するtaskは，`go-concourse-src/concourse/tasks/build.yaml`というyamlに書かれているのでみてみたいと思います．
```
        on_success:
          do:
            - put: go-concourse-status
...
## pipelineの登録
ログイン時にダウンロードしたflyコマンドでpipelineを登録していきます，  
CLIで，pipelineの登録，実行，削除などができるのもconcourseの魅力の一つだと思います．  
まず，CLI上でもログインをします．  
```
$ fly -t main login --concourse-url https://((concourseのドメイン))
```
targetは，誤った環境に対してコマンドを実行するリスクを減らすために定義しないといけません．今回は`main`で設定します．  
...
concourseのwebUIは，pipelineの状態，一つ一つのpipelineがどのようなresourseを取得し，どんなresourseを更新するか一目で分かりやすくなっています．  
さらに，pipelineをyamlで管理でき，操作がCLIで行えるので自分はとても気に入っております．
今回は，helmでconcourseの環境を構築しました，
私はconcourseを新しいクラスタではなく，dev環境のクラスタのconcourseというnamespaceで配置を行ったのですが，
devのmachine-typeは全てn1-standard-1をしようしていたので，n1-standard-2のmachine-typeを使用してpoolを作成する必要がありました(n1-standard-1だとCPU不足でうまくいかないことがありました...)，
ただ，concourseのhelm chartには，pod affinityの設定はあったのですが，node affinityを設定できるvalueがなかったので，とても悩みました．
concourseに限ったことではないですが，stableのchartはとても便利でいいと思うのですが，自分好みのマニフェストにしたいときは結局自分でマニフェストを書いて自分で管理した方がいいのか考えるきっかけになりました．
